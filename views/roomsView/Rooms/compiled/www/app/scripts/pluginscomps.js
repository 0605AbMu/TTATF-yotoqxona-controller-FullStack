...
Vue.component('decsoft-selectex', {
  ...
  template: '\
   <select v-bind:id="name" v-bind:tabindex="tabIndex" v-show="!hidden" v-bind:class="[classes, \'form-select\', \'form-select-\' + size]" v-bind:title="title" v-bind:disabled="disabled" @change="changeHandler" v-on:focus="focusHandler" v-on:blur="blurHandler" v-on:click="clickHandler" v-on:dblclick="dblclickHandler" v-on:mousedown="mousedownHandler" v-on:mouseup="mouseupHandler" v-on:mousemove="mousemoveHandler" v-on:mouseenter="mouseenterHandler" v-on:mouseleave="mouseleaveHandler" v-on:contextmenu="contextmenuHandler">\
